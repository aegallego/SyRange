...
function enginestep() {
	...
	if (!gameover) {
        ...
        if (ctx.hello>=500) {
            ctx.levels=leveling;
            ctx.fillText("Level:   "+ctx.levels,20,120);
        } else {
	    	ctx.fillText("Level:   "+leveling,20,120);
		}
	    if (health>30) {
        ...
        ctx.hello = score,stage.w/2,stage.h/2;
        if (ctx.hello>=500) {
	    	ctx.fillText("NEXT STAGE",stage.w/2,stage.h*0.65);
	    }else {ctx.fillText("PLAY AGAIN",stage.w/2,stage.h*0.65);}
		...
		for (var i = 0; i < cannons.length; i++) {
            ...
            if (can.r<1&&gameover) {
				...
				if (can.x>(stage.w-220)/2&&can.y>stage.h*0.65-25) {
                    if  (ctx.hello>=500) {
					againprog +=1;
					if (againprog>220&&score>=500) {
                        leveling+1;
						nextGame();
						gameover = false;
					}
